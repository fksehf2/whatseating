...
public class MenuDAO {
	...
	public int menuRegister(MenuDTO menuDTO) {
		int insertResult = session.insert("mappers.menuMapper.menuRegister",menuDTO);
		...
		for(int i=0; i < list.size(); i++) {
			...
			picInsertResult = session.insert("mappers.menuMapper.menu_picRegister", fileInfoDTO);
		}
	...
	public int menuPicDelete(String SAVEFILE) {
		int deleteResult = session.delete("mappers.menuMapper.menu_picDelete",SAVEFILE);
		return deleteResult;
	}
	...
	public List<MenuDTO> menuList(int ST_CODE){
		List<MenuDTO> menuList = session.selectList("mappers.menuMapper.menuList", ST_CODE);
	...
	public MenuDTO menuDetail(int MENU_NUM){
		MenuDTO menuDTO = session.selectOne("mappers.menuMapper.menuDetail", MENU_NUM);
	...
	public List<FileInfoDTO> picList(int MENU_NUM){
		List<FileInfoDTO> picList = session.selectList("mappers.menuMapper.picList", MENU_NUM);
	...
	public int menuModify(MenuDTO menuDTO) {
		int updateResult = session.update("mappers.menuMapper.menuModify", menuDTO);
		return updateResult;
	}
	
	//메뉴 삭제
	public int menuDelete(int MENU_NUM) {
		int deleteResult = session.delete("mappers.menuMapper.menuDelete",MENU_NUM);
		return deleteResult;
	}
