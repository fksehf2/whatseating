	...
	var contextPath = getContext;
				...
				$.ajax({
					...
					success : function(data){
						...
						$.each(menuResult,function(index,item){
							...
							body += "<img alt='사진없음' src='' class='menu-mainPic'>";
				...
				$(document).on("click",".menu-list-frame",function(){
					...
					$.ajax({
							...
							success : function(menuData){
								...
								$("#menuDetailForm").find("input[name='MENU_NUM']").attr("value",menuDTO.menu_NUM);
							...
							success : function(picdata){
								...
								if(picInfo.length == 0){
									...
									$.each(picInfo, function(idx, item){
										...
										originalUpload = "";
										originalUpload += "<div id='originalFile"+originalFileNum+"'>"+item.originalfile+"</div>";
								...
								$(document).on("change","#realPicUpload",function(){
									...
									modifyFilesArr.forEach(function(file){
										...
										reader.onload = function(){
											...
											var uploadFile = "";
											uploadFile += '<div id="file' + fileNum + '"class="addFile">'+file.name+'</div>'
		...
		$("#menuPicUpdateBtn").on("click",function(){
			...
			for(var i=0; i<modify_file.length; i++){
				formdata.append("modifyImg",modify_file[i]);
			}
			...
			$.ajax({
				...
				success : function(modifyResult){
					console.log(modifyResult.updateResult)
					if(modifyResult.updateResult == 1){
						alert("사진이 수정되었습니다.");
						$("#menuPicUpdateBtn").css("display","none");
						$("#menuPicModifyBtn").css("display","inline-block");
					}
